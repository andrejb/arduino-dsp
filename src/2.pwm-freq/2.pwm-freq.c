...
void fill_sinewave(){
  byte bb;
  int iw;
  for (iw = 0; iw < 512; iw++){// with 1 period of sinewawe
    fd     = 127*sin(fcnt);     // fundamental tone
    fcnt   = fcnt+dx;           // in the range of 0 to 2xpi and 1/512 increments
    dd[iw] = 127+fd;  // add dc offset to sinewave and write value into array
  }
}
...
void loop()
{
  ...
  //while ((1 & 1) == 0);
  //counter = 0;
  //while ((_SFR_BYTE(ADCSRA) & _BV(ADSC)) == _BV(ADSC))
  //  counter++;
  //period += micros() - stime;
  //ticks++;
  ...
  if ((time - ltime) >= 1000000) {
    ...
    Serial.println(((double)period)/ticks);
