...
#define BUFFER_SIZE (4*BLOCK_SIZE)
...
void loop()
{
  ...
  for (int n = rind; n < maxind; n++) {
    //y[n&(BUFFER_SIZE-1)] = x[n&(BUFFER_SIZE-1)];
    /* passos:
     *   1. subtrair offset: 127 - val.
     *   2. conta.
     *   3. adicionar offset
     */
    xn = 127 - x[n&(BUFFER_SIZE-1)];
    yn1 = 127 - x[(n-1)&(BUFFER_SIZE-1)];
    yn = (xn + yn1);
    LIMIT(yn, -127, 127);
    yn >>= 3;
    yn += 127;
    //y[n&(BUFFER_SIZE-1)] = x[n&(BUFFER_SIZE-1)];
    y[n&(BUFFER_SIZE-1)] = (uint8_t) yn;
  }
  ...
  //  Serial.print(convtime);
    ...
    Serial.print(yn-127);
    Serial.print("  convs: ");
    Serial.println(convs);
  //  Serial.print("  mean conv time: ");
  //  Serial.print(((double)convtime)/convs);
  //  Serial.print("  val: ");
